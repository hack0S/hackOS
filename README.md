[![temp-Image11bs-U4.avif](https://i.postimg.cc/6pjVNHJH/temp-Image11bs-U4.avif)](https://postimg.cc/LJPgt3sf)

# hackOS
hackOS is a FreeBSD based distribution that is **right about to release** for x86.
It is made for replacing a hackintosh, as macOS 26 is the last one to support intel.

## NOTICE
The OS is optimized for laptops, so it can have some cosmetic issues with a desktop.

## Compatibility
Currently, only native FreeBSD apps would be supported, on later versions, it is planned to be compatible with *some* macOS apps.

## Cosmetics
Not decided currently.

## Release
It is planned to release before **25th October** as it is currently in development.
First version (Tahoma) might be based on [ravynOS](https://ravynos.com), just as the beta builds but, hopefully, not.

## Future of the project
hackOS will be updated *probably: if the project won't fail* and i will try to develop my own software for it.
